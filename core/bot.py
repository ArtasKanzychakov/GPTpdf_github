...
class BusinessNavigatorBot:
    ...
    def setup_handlers(self):
        ...
        # Обработчики callback-запросов (кнопки) - ТОЛЬКО СПЕЦИФИЧНЫЕ
        # ИСПРАВЛЕНО: убрали глобальный CallbackQueryHandler
        ...
        # Обработчики для анкеты - делегируем questionnaire.py
        self.application.add_handler(CallbackQueryHandler(
            handle_button_answer, pattern="^(?!select_niche_|restart_questionnaire|show_profile)"
        ))
